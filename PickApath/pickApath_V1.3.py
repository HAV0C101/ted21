...
userStats = [10, 50, 25, 100, ["None", "Sword", "Arrow", "Hat", "Potion Of Healing"]]
"""
0 = health
1 = stamina
2 = dexterity 
    ...
    while enemyHP > 1:
        ...
        if userFightInput == "f":
            ...
            if rand == 0:
                ...
                userHealthTakeOff = random.randint(5, 20)
                print("You Were Unable to Attack The %s And lost %d HP in the Process" %(name, userHealthTakeOff))
                userStats[0] -= userHealthTakeOff
                print("Your Current Health: %d" % userStats[0])
        if userFightInput == "h":
            # print("You dont Currently Have That Ability \n Coming Soon™")
            if "Potion Of Healing" in userStats[4]:
                if userStats[0] > 25:
                    healAmount = 100 - userStats[0]
                    print(healAmount)
                    userStats[0] += healAmount
                else:
                    healAmount = 75
                    print(healAmount)
                    userStats[0] += healAmount
            else:
                print("You Dont Currently have anything to heal with")
        print("\n What do you wish to do (F)ight (R)un (C)heck Stats or (H)eal \n")
        userFightInput = input(">>>").lower()
